...
# Warm-up
let
    lin_sys, sagbi = oscillator_instance(1, 2)
    param = Parametrization(sagbi)
    println("detected weight = ", detect_weight(param))
    SagbiHomotopy.solve(LinearSection(lin_sys), param; check_degree = false)
end
...
for s in 1:N
    for r in 1:M
        lin_sys, sagbi = oscillator_instance(s, r; seed = 20260210 + s + 10r)
